<template>
  <div>
    <h2 class="text-xl font-serif text-center pt-24 mb-5">
      Find a Promising Candidate with us!
    </h2>
    <div class="mb-14"></div>
    <div class="grid grid-cols-4 gap-2">
      <div class="flex justify-between items-center col-end-5 col-span-3 mx-2">
        <div class="text-sm records">
          Showing {{ (page - 1) * 20 + 1 }} -
          {{ Math.min(page * 20, filteredStaffs.length) }} of
          {{ filteredStaffs.length }} staff
        </div>
        <Pagnition
          :maxPage="Math.max(1, Math.ceil(filteredStaffs.length / 20))"
          v-model="page"
        />
      </div>
      <div>
        <FilterComponent
          class="w-full ml-2"
          :countries="countries"
          :hiringDepartments="hiringDepartment"
      ...
      <div
        ...
        class="col-span-3 grid grid-cols-4 gap-3 mx-2 self-start"
      >
        <div
          class="white-card cursor-pointer"
          v-for="(staffInfo, indx) of filteredStaffs.slice(
            (page - 1) * 20,
            page * 20
          )"
        ...
        >
          <div
            v-show="allSkillsLength > 0"
            class="flex justify-end text-sm percentageMatch"
          >
            <div
              v-if="staffInfo.staff_skills.length > 0"
              class="bg-green rounded-lg text-white p-2"
            >
              {{
                (
                  (staffInfo.staff_skills.length / allSkillsLength) *
                  100
                ).toFixed(2)
              }}%
            </div>
            <div v-else class="bg-white p-2 text-white">0</div>
          </div>
          <div class="flex justify-center">
            <img :src="require(`@/assets/${getRandomInt()}.svg`)" />
          </div>
          <h4 class="mt-2 text-yellow text-lg font-semibold staff_name">
            {{ staffInfo.staff_FName }} {{ staffInfo.staff_LName }}
          </h4>
          <p class="text-sm mt-2 staff_ID">
            <i class="bi bi-person-badge"></i> {{ staffInfo.staff_ID }}
          </p>
          <p class="text-xs mt-2 staff_email">
            <i class="bi bi-envelope-at-fill mr-2"></i
            ><span class="text-grey">{{ staffInfo.email }}</span>
          </p>
          <div
            class="flex justify-center content-center text-center text-yellow mt-2"
          >
            <div class="font-bold staff_dept">{{ staffInfo.dept }}</div>
            <div
              class="bg-yellow mx-2 h-1 w-1 rounded-full place-self-center"
            ></div>
...
axios
  .get(`${backend_url.value}/get_all_skill`)
  .then((response) => {
...
axios
  .get(`${backend_url.value}/openrolelisting?manager_ID=${user.value.staff_ID}`)
  .then((response) => {
...
function updateFilter(data) {
  ...
  if (data.selectedRoleListing.length > 0) {
    axios
      .get(`${backend_url.value}/get_role_skill/${data.selectedRoleListing}`)
      .then((response) => {
...
const filteredStaffs = computed(() => {
  return staffs.value.filter((item) => {
    const countryFilter =
      selectedCountry.value === "all" || item.country === selectedCountry.value;
    const deptFilter =
      selectedDept.value === "all" || item.dept === selectedDept.value;
    const skillsFilter =
      selectedSkills.value.length === 0 ||
      item.staff_skills.some((skill) => selectedSkills.value.includes(skill));
